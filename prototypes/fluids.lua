...
for name, recipe in pairs(data.raw.recipe) do
   if (not recipe.hidden) and (recipe.enabled == nil or recipe.enabled or validRecipes[name] == 1) then
      ...
      if recipe.expensive then
         table.insert(checklocations, recipe.expensive)
      end
      if recipe.normal then
         table.insert(checklocations, recipe.normal)
      end
         ...
         if checklocation.ingredients then
            for _, ingredient in ipairs(checklocation.ingredients) do
               if ingredient[1] then
                  validItems[ingredient[1]] = 1
               end
               if ingredient.type == "item" then
                  validItems[ingredient.name] = 1
               end
            end
         end

         if checklocation.result then
            validItems[checklocation.result] = 1
         end

         if checklocation.results then
            if checklocation.results and type(checklocation.results) == "table" then      --add table
               for _, result in ipairs(checklocation.results) do
                  if result.type == "item" then
                     validItems[result.name] = 1
                  end
               end
            end
         end
...
for name, item in pairs(data.raw.item) do
   if item.rocket_launch_product then
      if item.rocket_launch_product[1] then
         validItems[item.rocket_launch_product[1]] = 1
      end
      if item.rocket_launch_product.name then
         validItems[item.rocket_launch_product.name] = 1
      end
   end
   ...
   if item.rocket_launch_product then
      for _, product in item.rocket_launch_product do
         if product[1] then
            validItems[product[1]] = 1
         end
         if product.name then
            validItems[product.name] = 1
         end
      end
   end
end
...
for _, tbl in ipairs({
   ...
   for name, item in pairs(tbl) do
      if string.find(name, "-barrel") and name ~= "empty-barrel" then
         goto continue
      end

      if string.find(name, "textplate") or string.find(name, "deadlock-crate") then
         goto continue
      end
      ...
      pneumaticFluid.hidden_in_factoriopedia = true      -- add hide in factoriopedia
      pneumaticFluid.base_color = {math.random(), math.random(), math.random()}     -- add random color fluids
      ...
      if item.icon ~= nil or item.icons ~= nil then
         ...
         table.insert(pneumaticFluid.icons, {
            icon = "__pneumatic-transport__/graphics/icon/fluids/water.png",
            icon_mipmaps = 4,
            icon_size = 64,
            scale = 0.25,
            shift = {12, 9}      --add shift
         })
      end
      ...
      if item.stack_size and quantity > item.stack_size / 5 then
         quantity = item.stack_size / 5
      end
      if quantity < 1 then
         quantity = 1
      end
      if heavyItems[item.name] then
         quantity = 1
         fluidSize = 100
      end
      ...
      table.insert(pneumaticRecipes, {
         ...
         localised_name = {"recipe-name.pneumatic-liquify", item.localised_name or {"item-name." .. item.name}},     --add localised_name recipes
         energy_required = 0.4,
         enabled = true,
         hidden_in_factoriopedia = true,     --add hide in factoriopedia
      ...
      table.insert(pneumaticRecipes, {
         ...
         localised_name = {"recipe-name.pneumatic-solidify", item.localised_name or {"item-name." .. item.name}},     --add localised_name recipes
         energy_required = 0.4,
         enabled = true,
         hidden_in_factoriopedia = true,     --add hide in factoriopedia
         hide_from_player_crafting = true,
